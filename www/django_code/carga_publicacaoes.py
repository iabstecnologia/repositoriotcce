        ...
        with transaction.atomic():
            for i, item_data in enumerate(itens_para_importar):
                try:
                    ...
                    # Autores: apenas vincula autores já existentes no banco
                    autores_names = [a.strip() for a in item_data.get("AUTOR", "").split(',') if a.strip()]
                    for nome in autores_names:
                        autores = list(Autor.objects.filter(nome=nome))
                        if not autores:
                            raise ValueError(f"Autor não encontrado: '{nome}'.")
                        for autor in autores:
                            registro.autores.add(autor)

                    # Tags: apenas vincula tags já existentes no banco
                    tags_names = [t.strip().replace(',', '').replace('.', '') for t in item_data.get("TAGS", "").split(';') if t.strip()]
                    for t_nome in tags_names:
                        tags = list(Tag.objects.filter(nome=t_nome))
                        if not tags:
                            raise ValueError(f"Tag não encontrada: '{t_nome}'.")
                        for tag in tags:
